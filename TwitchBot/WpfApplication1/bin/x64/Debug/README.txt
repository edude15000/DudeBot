DudeBot is a TwitchBot made by Edude15000.
*****YOU WILL NEED .NET 4.6 or higher.

******************************************************

*****Preparing DudeBot
1. Unpackage application to an easy to manage location (like Desktop).
2. Ensure you have .NET 4.6 or higher installed on your computer.
	You can install it free here: 
	https://www.microsoft.com/en-us/download/details.aspx?id=44928
3. Create a new Twitch account for your bot (Unless you already have one to use)!
4. Ensure that your bot is logged into Twitch.
5. Run 'DudeBot.exe', you should get a first time user message.
6. Go to: http://www.twitchapps.com/tmi/ and Connect the bot with Twitch, 
	then copy the whole oauth code from the text box into the Oauth text block.
	Example : oauth:d9r7onmy0apujm8odkn20zq02w8915
7. Enter your Bot Name and Streamer Name into the appropriate fields.
8. Scroll to the bottom of the customization tab and press 'Apply', 
	the GUI will restart and populate data, if not manually restart the bot. 
	If the bot is failing to connect, please enter correct data for the
	three fields, click 'Apply', and restart the bot.
9. You are ready to go!

NOTE: If the bot is crashing when you try to open it but you already have 
	.NET up to date, make sure you give permissions to dudebot.exe and 
	dudebotupdater.exe in your antivirus. 
	The bot will also copy the dudebotupdate.exe file to your %temp% directory, 
	so if you are still having problems, make sure to give that file permissions 
	to run as well.

NOTE: MAKE SURE DUDEBOT ACCOUNT IS MODDED / SET AS BOT IN CHANNEL!

*****OBS Integration
10. In OBS, create new 'text' boxes, and in the 'Use Text From File',
	navigate to 'currentsong.txt' and 'currentrequester.txt'. 
	These will dynamically show the currently playing song and requester on stream!
	
*****Images on Stream OBS STUDIO integration
12. Go to the Dashboard tab and click 'open images window.' 
	In OBS STUDIO, create a new 'window capture' and in the Window dropbox, 
	select '[Images.exe]: Images.'
	Leave Window Match Priority as Window Title. 
	Right click on the newly created window capture and click 'filters.' 
	Click on the '+' at the bottom left to add a new filter.
	Click 'chroma key' and for 'Key Color Type' select Green, 
	then select 1 for Similarity, Smoothness, and Key Color Spill Reduction.
	You can check the 'Open window on start' checkbox so that it will automatically 
	open the images window every time you start DudeBot.
...
13. Go to https://docs.google.com/spreadsheets/u/0/ and create a new Google Sheet.
	You should have a link that looks like: 
	https://docs.google.com/spreadsheets/d/1BxiMVs0XRAvE2upms/edit.
	Take the middle section and copy it (1BxiMVs0XRAvE2upms) 
	and paste it into the Google Sheet ID textbox on the Customization page. 
	Close and restart DudeBot. You should get a pop up to login with your 
	Google Account.
...
***Dudebot Commands
(All commands used are defaults)
NOTE: There are three request lists (that apply in the following order): 
	$$$(Donation), VIP, REG
...
!addtop artist - song (requesterName) = Add a song to the top of the song list 
	using the requesterName as the requester
!adddonator artist - song = Add a donation song
!adddonator artist - song (requesterName) = Add a donation song using the 
	requesterName as the requester
...
!addvip artist - song = Add a VIP song to the VIP list 
	(will keep the current playing song in the first postion)
!addvip artist - song (requesterName) = Add a VIP song to the VIP list using 
	the requesterName as the requester (will keep the current playing song in the 
	first position)
...
!regnext OR !nextreg OR !regularnext OR !nextregular = *If there exists a standard 
	request in the list, it will move it to the now playing song 
	(overwriting the previous now playing song), 
	otherwise it will just call the !next command (CANT CHANGE COMMAND NAME!)
!botcolor color = Changes color of bot in chat 
	(must be Twitch official color, MOD / STREAMER ONLY)
!removesong # = Deletes song in song place # (MOD / STREAMER ONLY)

******************************************************************************************************************************************

***Custom Commands
To add a custom command, type the command name in the first box, 
	(!kappa) and then the response in the second box (Kappa).
To edit a custom command, click on the command in the list, edit the response 
	and press 'Add/Edit'.
To delete a custom command, click on the command in the list, then press 'Delete'.

***Custom Variables
* If you use these variables in your commands, they will display the 
	following response:
...
$shoutout = Displays generic shoutout message using following text as user 
	(MOD ONLY)!
...
$streamplays = Number of songs played so far this stream
$lifetimeplays = Number of total songs played
$userrank = User's rank
$usertime = User's time spent in stream
$usersubcredits = User's amount of subcredits
$userpoints = User's number of points
$usernumrequests = User's number of requests
$usermonths = User's number of consecutive months subbed for
$currentsonglevel = Current playing song level
...
Example:	!harambe	Harambe would have loved $currentsong!
...
!givecredits @user amount = Gives to / takes sub credit amount to / 
	from user (mods, streamer ONLY)
...
Overall cooldown time (seconds) : Cooldown between image displays

***Quotes System
All quote system commands are hard-coded!
!quotes on / off = Toggles quote system on / off. STREAMER ONLY
!addquote quote = Adds the quote using the streamer as the quoter. 
	(!quoteadd also works) MOD AND STREAMER ONLY
!addquote quote (user) = Adds the quote using the 'user' as the quoter. 
	(!quoteadd also works) MOD AND STREAMER ONLY
...
!endgame # = Ends the game with the number being the winning number 
	(whoever guessed closest wins!). MOD AND STREAMER ONLY
!endgame OR !cancelgame = Ends game without a winner

***Adventure Game
!adventure on / off = Toggles adventuring
!adventure = starts/joins an adventure
User Join Time : Time allowed for other users to join the adventure after started 
	(seconds)
...
If the VIP song reward in revlobot starts with "VIP" and the command to cash in 
	points to get the VIP song reward starts with "VIP"
	then Dudebot will parse Revlobot's response and add the VIP to its 
	list automatically!
